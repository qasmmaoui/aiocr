#!/bin/bash
# Restart API + UI only (Ollama & Qdrant left running). Launch:
#   setsid bash restart_app.sh >/workspace/ui.log 2>&1 </dev/null &
...
sleep 4
exec /workspace/venv/bin/streamlit run app.py --server.port 8888 --server.address 0.0.0.0 \
